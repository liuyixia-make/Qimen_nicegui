...
class 奇门遁甲:
        ...
        if len(完整旬首) >= 3:  # 确保旬首格式正确，如"甲子戊"
            ...
            if 找到宫位:
                ...
                if 找到宫位 == "五宫":
                    ...
                    print(f"找到值符天干 {值符天干} 在五宫，记录为二宫，值符填入天禽，值使填入死门")
                else:
                    self.奇门遁甲数据["起局信息"]["旬宫"] = self.奇门遁甲数据[找到宫位].get("宫数", "")
                    本位星 = self.奇门遁甲数据[找到宫位].get("本位星", "")
                    本位门 = self.奇门遁甲数据[找到宫位].get("本位门", "")
                    self.奇门遁甲数据["起局信息"]["值符"] = 本位星
                    self.奇门遁甲数据["起局信息"]["值使"] = 本位门
                    print(f"找到值符天干 {值符天干} 在 {找到宫位}，值符填入 {本位星}，值使填入 {本位门}")
            else:
                print(f"警告：未能找到值符天干 {值符天干} 在九宫地盘干中的位置")
    ...
    def 排九星(self):
        ...
        # [这里添加从排神煞移过来的计算值符宫位信息的代码]
        # 计算"值符"的宫位信息
        旬首 = self.奇门遁甲数据["起局信息"]["旬首"]
        if len(旬首) >= 3:  # 确保旬首格式正确，如"甲子戊"
            值符天干 = 旬首[-1]  # 取最后一个字作为值符天干
            值符 = self.奇门遁甲数据["起局信息"]["值符"]  # 获取已经计算好的值符
    ...
    def 排八门(self):
        ...
        2. 取值使门为B（如休门、生门等）
        3. 取旬宫数字为C
        4. 根据时柱天干在天干序列中的位置，从旬宫开始排列
           - 甲对应旬宫位置
           - 如果是阳遁，按顺序排列（如旬宫是3，乙就排在4宫）
           - 如果是阴遁，按逆序排列（如旬宫是3，乙就排在2宫）
        5. 一直排到当前日柱天干所在位置，将值使门填入该宫位的"天盘门"字段
        6. 按照八门顺序：[开门，休门，生门，伤门，杜门，景门，死门，惊门]，从值使门所在宫位开始顺时针排列其他门
        """
        # 八门旺衰表
        八门旺衰表 = {
            "开门": {
        ...
        日柱天干 = self.奇门遁甲数据["起局信息"]["四柱干支"][2][0]
        
        # 获取月柱地支（用于计算天盘门旺衰）
        月柱地支 = ""
        try:
            月柱地支 = self.奇门遁甲数据["起局信息"]["四柱干支"][1][1]  # 月柱地支
            print(f"月柱地支: {月柱地支}")
        except:
            print("获取月柱地支失败，使用默认值")
            月柱地支 = "寅"  # 默认值
        ...
        # 找到日柱天干在天干序列中的索引
        日柱天干索引 = 天干序列.index(日柱天干) if 日柱天干 in 天干序列 else None
        
        if 时柱天干索引 is None or 日柱天干索引 is None:
            print(f"警告：找不到时柱天干 {时柱天干} 或日柱天干 {日柱天干} 在天干序列中的索引")
            return False
        ...
        # 将值使门填入日柱天干宫位的"天盘门"字段，不带宫数
        if 日柱天干宫位键 in self.奇门遁甲数据:
            self.奇门遁甲数据[日柱天干宫位键]["天盘门"] = 值使门
            print(f"将值使门 {值使门} 填入 {日柱天干宫位键}")
            
            # 更新起局信息中的值使，加上宫数（阿拉伯数字）
            阿拉伯宫数 = 汉字数字映射.get(日柱天干宫位, "")
            if 阿拉伯宫数:
                值使门带宫数 = f"{值使门}{阿拉伯宫数}宫"
                self.奇门遁甲数据["起局信息"]["值使"] = 值使门带宫数
                print(f"更新起局信息的值使为: {值使门带宫数}")
        else:
            print(f"警告：找不到宫位键 {日柱天干宫位键} 在奇门遁甲数据中")
            return False
        
        # 从值使门开始，按照顺时针顺序排列其他门
        # 修正：增加一个偏移量，解决少转一位的问题
        偏移量 = 1  # 增加一个偏移量
        for i in range(8):
            # 计算当前门的索引，从值使门开始
            当前门索引 = (值使门索引 + i) % 8
            # 计算当前宫位的索引，从日柱天干宫位索引开始，加上偏移量
            当前宫位索引 = (日柱天干宫位索引 + i + 偏移量) % 8
            # 获取当前门和宫位
            当前门 = 八门序列[当前门索引]
            当前宫位 = 宫位序列[当前宫位索引]
            当前宫位键 = 宫位映射[当前宫位]
            
            # 将当前门填入当前宫位的"天盘门"字段，不带宫数
            if 当前宫位键 in self.奇门遁甲数据:
                self.奇门遁甲数据[当前宫位键]["天盘门"] = 当前门
                
                # 计算天盘门旺衰
                if 当前门 in 八门旺衰表:
                    # 获取当前宫位对应的八卦名
                    八卦名 = 宫位八卦映射.get(当前宫位键, "")
                    
                    # 计算与月柱地支的旺衰关系
                    月柱旺衰 = ""
                    月柱查询值 = f"{月柱地支}月"
                    
                    if 月柱查询值 in 八门旺衰表[当前门]["旺"]:
                        月柱旺衰 = "旺"
                    elif 月柱查询值 in 八门旺衰表[当前门]["相"]:
                        月柱旺衰 = "相"
                    elif 月柱查询值 in 八门旺衰表[当前门]["休"]:
                        月柱旺衰 = "休"
                    elif 月柱查询值 in 八门旺衰表[当前门]["囚"]:
                        月柱旺衰 = "囚"
                    elif 月柱查询值 in 八门旺衰表[当前门]["死"]:
                        月柱旺衰 = "死"
                    
                    # 计算与宫位的旺衰关系
                    宫位旺衰 = ""
                    
                    if 八卦名 in 八门旺衰表[当前门]["旺"]:
                        宫位旺衰 = "旺"
                    elif 八卦名 in 八门旺衰表[当前门]["相"]:
                        宫位旺衰 = "相"
                    elif 八卦名 in 八门旺衰表[当前门]["休"]:
                        宫位旺衰 = "休"
                    elif 八卦名 in 八门旺衰表[当前门]["囚"]:
                        宫位旺衰 = "囚"
                    elif 八卦名 in 八门旺衰表[当前门]["死"]:
                        宫位旺衰 = "死"
                    
                    # 更新天盘门旺衰
                    self.奇门遁甲数据[当前宫位键]["天盘门旺衰"] = [月柱旺衰, 宫位旺衰]
                    print(f"{当前宫位键} 的 {当前门} 旺衰: [月柱:{月柱旺衰}, 宫位:{宫位旺衰}]")
                else:
                    # 如果找不到门的旺衰信息，使用空值
                    self.奇门遁甲数据[当前宫位键]["天盘门旺衰"] = ["", ""]
                
                print(f"将 {当前门} 填入 {当前宫位键}")
                
                # 如果是第一个门（值使门），更新起局信息中的值使
                if i == 0:
                    # 获取正确的宫位对应的阿拉伯数字
                    阿拉伯宫数 = 汉字数字映射.get(当前宫位, "")
                    if 阿拉伯宫数:
                        值使门带宫数 = f"{当前门}{阿拉伯宫数}宫"
                        self.奇门遁甲数据["起局信息"]["值使"] = 值使门带宫数
                        print(f"更新起局信息的值使为: {值使门带宫数}")

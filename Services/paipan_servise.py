...
class 奇门遁甲:
    ...
    def 排地盘干(self):
        ...
        try:
            ...
            print(f"[DEBUG] 排地盘干 - 遁: {遁}, 局数: {局数}")
        except Exception as e:
            print(f"解析局数信息失败: {e}")
            return False
            
        # 检查奇门遁甲数据中的宫位键
        print(f"[DEBUG] 排地盘干 - 奇门遁甲数据中的键: {list(self.奇门遁甲数据.keys())}")
    ...
    def 排盘(self):
        ...
        for 宫位键 in ["一宫", "二宫", "三宫", "四宫", "五宫", "六宫", "七宫", "八宫", "九宫"]:
            if 宫位键 in self.奇门遁甲数据:
                print(f"[DEBUG] {宫位键}: 地盘干 = {self.奇门遁甲数据[宫位键].get('地盘干', '无')}")
